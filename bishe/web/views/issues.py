            ...
            if name == 'assign':
                ...
                if name == 'issues_type':
                    if value in ["1", "2", "3"]:
                        ...
                        instance = field_object.remote_field.model.objects.filter(id=value,
...
def invite_join(request, code):
    ...
    # 是否已过期，如果已过期则使用免费额度
    max_transaction = models.Transaction.objects.filter(user=invite_object.project.creator, status=2).order_by('-id').first()
    if not max_transaction:
        max_member = models.PricePolicy.objects.filter(category=1).first().person_num
    else:
        if max_transaction.end_datetime < current_datetime:
            free_object = models.PricePolicy.objects.filter(category=1).first()
            max_member = free_object.person_num
        else:
            max_member = max_transaction.price_policy.person_num
    ...
    models.ProjectUser.objects.create(user=request.tracer.user, project=invite_object.project, invitee=invite_object.project.creator)
